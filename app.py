...
class Accounts:
    ...
    def create_label_frame(self):
        ...
        Label(labelframe, text="Name : ", bg="green",
              fg="white").grid(row=1, column=1, sticky=W, pady=2, padx=15)
    ...
    def Add_account(self):
        if self.add_account_v():
            ...
            query = 'CREATE TABLE {} ("no"	INTEGER,"Credit"	NUMERIC,"Debit"	INTEGER,"Date"	date,PRIMARY KEY("no" AUTOINCREMENT))'.format(
                s)
            self.execute_db_query(query)
            query = 'INSERT INTO Main(name) VALUES("{}")'.format(
                self.namefield.get())

            self.execute_db_query(query)
            self.message["text"] = "New account {} added".format(
                self.namefield.get())
    ...
    def open_modify_window(self):
        ...
        labelframe = LabelFrame(self.window,
                                text="{}".format(name),
                                bg="sky blue",
                                font="helvetica 10")
        labelframe.grid(row=0, column=0, padx=8, pady=8, sticky="ew")
        Label(labelframe, text="Date : ", bg="green",
              fg="white").grid(row=1, column=1, sticky=W, pady=2, padx=15)
        self.namefield = Entry(labelframe)
        ...
        Label(labelframe, text="Credit : ", bg="green",
              fg="white").grid(row=2, column=1, sticky=W, pady=2, padx=15)
        self.creditfield = Entry(labelframe)
        self.creditfield.grid(
            row=2,
            column=2,
            sticky=W,
            padx=5,
        )
        Label(labelframe, text="Debit : ", bg="green",
              fg="white").grid(row=3, column=1, sticky=W, pady=2, padx=15)
        self.Debitfield = Entry(labelframe)
        self.Debitfield.grid(
            row=3,
            column=2,
            sticky=W,
            padx=5,
        ...
        for item in items:
            self.tree.delete(item)
        query = "SELECT * FROM {} order by Date desc".format(
            name.replace(" ", "_"))
        contact_entries = self.execute_db_query(query)
        for row in contact_entries:
            self.tree.insert("",
                             0,
                             text=row[1],
                             values=(row[2], row[3], row[0], row[1]))

        Button(
            text="Delete Selected",
            command=lambda: self.delete_data(name.replace(" ", "_")),
            bg="red",
            fg="white",
        ).grid(row=8, column=0, sticky=W, pady=10, padx=10)
        Button(
            text="Edit selected",
            command=lambda: self.open_modify(name.replace(" ", "_")),
            bg="purple",
            fg="white",
        ).grid(row=8, column=1, sticky=W)
        Button(
            text="Exoprt to Excel",
            command=lambda: self.export_to_exel(name.replace(" ", "_")),
            bg="blue",
            fg="white",
        ).grid(row=9, column=1, sticky=W)
        Button(text="Back", command=self.call_main, bg="orange",
               fg="Black").grid(row=9, column=0, sticky=W, pady=10, padx=10)
    ...
    def open_modify(self, name):
        ...
        Button(
            self.win,
            text="Update Contact",
            command=lambda: self.update_contacts(d.get(), date.get(), c.get(),
                                                 index, name),
    ...
    def update_contacts(self, d, date, c, index, name):
        query = (
            "UPDATE {0} SET credit = ? , debit = ? , date = ? where no = {1}".
            format(name, index))
        parameters = (c, d, date)
    ...
    def export_to_exel(self, name):
        ...
        conn = sqlite3.connect(
            "C:/Users/lenovo/Desktop/Space Invaders GAME/projects/app/App.db")

        df = pd.read_sql("SELECT * FROM {}".format(name), conn)
        writer = pd.ExcelWriter(
            "C:/Users/lenovo/Documents/app_data/{}.xlsx".format(name))
